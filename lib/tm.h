...
__attribute__((aligned(64))) volatile unsigned int stalled;
__attribute__((aligned(64))) volatile unsigned int peak;
__attribute__((aligned(64))) volatile unsigned int last_commits;
...
#  define TM_THREAD_EXIT() { \
	...
	if (!myThreadId) { \
		printf("thread %i exiting. setting quota to %i\n", myThreadId, NUMBER_THREADS); \
...
# define PRINT_CLOCK_THROUGHPUT(CLOCKS) { \
		printf("Clock throughtput = %f\n", (float)(1000000 * normalized_commits)/(float)(CLOCKS)); \
	}
